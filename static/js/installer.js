// Монтажник: бонусный счёт, шкала, выбор скидки, списание бонусов
export function renderInstaller(user, onUpdate) {
    ...
    const maxBonusGoal = 1000;
    let bonusPercent = Math.min((bonus / maxBonusGoal) * 100, 100);
    let currentType = user.installerDiscountType || 'discount10';
    ...
    container.innerHTML = `
        <div class="card">
            ...
            <div class="option-group">
                ...
                <div data-type="cashback5" class="option-btn ${currentType === 'cashback5' ? 'active' : ''}">💰 Только кэшбэк 5%</div>
            </div>
        </div>
        <div class="card">
    ...
    document.querySelectorAll('.option-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (type) {
                user.installerDiscountType = type;
                onUpdate(user);
            }
        });
    });
    ...
    document.getElementById('addBonus100')?.addEventListener('click', () => {
        user.bonusPoints = (user.bonusPoints || 0) + 100;
        onUpdate(user);
    });
    document.getElementById('addBonus500')?.addEventListener('click', () => {
        user.bonusPoints = (user.bonusPoints || 0) + 500;
        onUpdate(user);
    });
    document.getElementById('addCustomBonusBtn')?.addEventListener('click', () => {
        ...
        if (!isNaN(val) && val > 0) {
            user.bonusPoints = (user.bonusPoints || 0) + val;
            onUpdate(user);
        }
    });

    // Списание бонусов (генерирует QR и обновляет баланс)
    document.getElementById('spendBonusQRBtn')?.addEventListener('click', () => {
        ...
        let currentBonus = user.bonusPoints || 0;
        ...
        // Генерация QR на списание (будет вызвана через колбэк из app.js)
